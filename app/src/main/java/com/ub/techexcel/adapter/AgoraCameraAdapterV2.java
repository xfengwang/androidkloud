...
public class AgoraCameraAdapterV2 extends RecyclerView.Adapter<AgoraCameraAdapterV2.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        if (holder.vedioFrame.getChildCount() == 0) {
            ...
            Log.e("check_selecr_coach", "meeting_config:" + meetingConfig);
                ...
                if (meetingConfig.isMeetingPause()) {
                    if (meetingConfig.isCoaching()) {
                        if (user.isCoachingStudent()) {
                            holder.coachStudentImage.setImageResource(R.drawable.icon_private_coach);
                            holder.coachAudienceImage.setImageResource(R.drawable.shape_transparent);
                            holder.coachToBeAudienceImage.setImageResource(R.drawable.shape_transparent);
                        } else if (user.isCoachingAudience()) {

                            holder.coachStudentImage.setImageResource(R.drawable.shape_transparent);
                            holder.coachAudienceImage.setImageResource(R.drawable.icon_coach_audience);
                            holder.coachToBeAudienceImage.setImageResource(R.drawable.shape_transparent);

                        } else if (user.isCoachingTobeAudience()) {

                            holder.coachStudentImage.setImageResource(R.drawable.shape_transparent);
                            holder.coachAudienceImage.setImageResource(R.drawable.shape_transparent);
                            holder.coachToBeAudienceImage.setImageResource(R.drawable.icon_coach_to_be_audience);
                        ...
                        holder.coachAudienceImage.setImageResource(R.drawable.shape_transparent);
                        holder.coachToBeAudienceImage.setImageResource(R.drawable.shape_transparent);
                        if (user.isSelectedCoach()) {
                            holder.selectMemberContainer.setBackgroundResource(R.drawable.bg_select_member);
                            holder.selectCoachImage.setImageResource(R.drawable.icon_select_coach);
                        } else {
                            holder.selectMemberContainer.setBackgroundResource(R.drawable.shape_transparent);
                            holder.selectCoachImage.setImageResource(R.drawable.shape_transparent);
                        }
                    }
    ...
    public void setMembers(List<AgoraMember> users) {
        if (users != null) {
            ...
            for (AgoraMember user : users) {
                if(user.getUserId()==Integer.parseInt(meetingConfig.getPresenterId())){
...
//            Collections.sort(users);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        public ViewHolder(View view) {
            ...
            selectCoachImage = view.findViewById(R.id.icon_select_coach);
            coachStudentImage = view.findViewById(R.id.icon_coach_student);
            coachAudienceImage = view.findViewById(R.id.icon_coach_audience);
            coachToBeAudienceImage = view.findViewById(R.id.icon_coach_audience_to_be_confirmed);
        }
...
//        Log.e("refreshMyAgoraStatus","notifyItemChanged,user:" + this.users.get(index));
    ...
    public void clearSelectedCoachMember() {
        ...
        if (speakerMember != null) {
            speakerMember.setSelectedCoach(false);
            notifyItemChanged(0);
        }
    ...
    public void setCoachStudent(AgoraMember studentMember) {
        int index = this.users.indexOf(studentMember);
        if (index >= 0) {
            this.users.get(index).setCoachingStudent(true);
            this.users.get(index).setCoachingAudience(false);
            this.users.get(index).setCoachingTobeAudience(false);
            notifyItemChanged(index);
        }
    }

    public void setCoachToBeAudience(AgoraMember studentMember){
        int index = this.users.indexOf(studentMember);
        if(index >= 0){
            this.users.get(index).setCoachingTobeAudience(true);
            this.users.get(index).setCoachingAudience(false);
            this.users.get(index).setCoachingStudent(false);
            notifyItemChanged(index);
        }
    }

    public void setCoachAudience(String userId) {
        int index = this.users.indexOf(new AgoraMember(Integer.parseInt(userId)));
        if(index >= 0){
            this.users.get(index).setCoachingAudience(true);
            this.users.get(index).setCoachingTobeAudience(false);
            this.users.get(index).setCoachingStudent(false);
            notifyItemChanged(index);
        }
    ...
    public void clearCoach(){
        for (AgoraMember user : users) {
            user.setCoachingAudience(false);
            user.setCoachingStudent(false);
            user.setCoachingTobeAudience(false);
            user.setSelectedCoach(false);
        }
        notifyDataSetChanged();
    }
