...
public class SwitchTeamActivity extends Activity implements View.OnClickListener {
    ...
    public static int RESULT_SWITCH=0X009;
    ...
    private void SaveSchoolInfo() {
        ...
        editor.commit();
        if(!getIntent().getBooleanExtra("FromSpace",false)){
            EventBus.getDefault().post(new TeamSpaceBean());
        }
        Intent intent=new Intent();
        intent.putExtra("teamid",teamSpaceBeans.getItemID());
        intent.putExtra("name",teamSpaceBeans.getName());
        setResult(RESULT_SWITCH,intent);
        finish();
    }
    ...
    private void initView() {
        ...
        titleText.setText(getResources().getString(R.string.switch_team));
    }
    ...
    public void recycle(TeamSpaceBean teamSpaceBean) {
