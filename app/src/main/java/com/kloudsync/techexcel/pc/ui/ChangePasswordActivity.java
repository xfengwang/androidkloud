...
public class ChangePasswordActivity extends Activity {
	...
	private void equalJudgment() {
		...
		int newPwdLength = new_password.length();
		...
		int hasNumber = new_password.matches(NUMBER) ? 1 : 0;
		int hasCase = new_password.matches(CASE) ? 1 : 0;
		int hasSymbol = new_password.matches(REGSYMBOL) ? 1 : 0;
		boolean mBool=newPwdLength > 7 && newPwdLength < 15 && hasNumber + hasCase + hasSymbol >= 2 && newPwdLength > 0;
		if (old_password == null || old_password.equals("")) {
			...
			Toast.makeText(getApplicationContext(), getResources().getString(R.string.password_rules), Toast.LENGTH_SHORT).show();
