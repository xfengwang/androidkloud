...
public class NoteViewActivityV2 extends BaseDocAndMeetingActivity implements PopBottomMenu.BottomMenuOperationsListener, PopBottomFile.BottomFileOperationsListener, AddFileFromFavoriteDialog.OnFavoriteDocSelectedListener,
		...
		, BottomDocThumbnailAdapter.DocThumbnailListener {
	private final String TAG = NoteViewActivityV2.class.getSimpleName();
	...
	public synchronized void receiveSocketMessage(EventSocketMessage socketMessage) {
		...
		switch (action) {
			...
			case SocketMessageManager.MESSAGE_MAKE_PRESENTER:
				...
				EventBus.getDefault().post(refreshMembers);
				break;
			case SocketMessageManager./*MESSAGE_ATTACHMENT_UPLOADED*/ATTACHMENT_UPLOADED_WITH_CHANGE_NUMBER:
				if (socketMessage.getData() == null) {
					return;
				}
	...
	public void videoPlayFunction(final int vid) {
		...
		if (meetingConfig.getType() == MeetingType.MEETING) {
			if (!meetingConfig.getPresenterId().equals(AppConfig.UserID)) {
				Toast.makeText(this, getResources().getString(R.string.no_operated), Toast.LENGTH_SHORT).show();
				return;
			}
		}
...
//                reloadAgoraMember();
	...
	private void openCameraForAddDoc() {
		if (!isCameraCanUse()) {
			Toast.makeText(getApplicationContext(), getResources().getString(R.string.camera_not_available), Toast.LENGTH_SHORT).show();
			return;
		}
