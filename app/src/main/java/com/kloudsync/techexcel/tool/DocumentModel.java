...
public class DocumentModel {
    ...
    public static void asyncGetDocumentsInDocAndShowPage2(final MeetingConfig meetingConfig, final boolean needShow) {
    ...
    public static void asyncGetDocumentsInDocAndShowPage(final MeetingConfig meetingConfig, final boolean needShow) {
        new ApiTask(new Runnable() {
            @Override
            public void run() {
                JSONObject response = ConnectService.getIncidentbyHttpGet(AppConfig.URL_MEETING_BASE + "meeting_document/list?lessonId=" + meetingConfig.getLessionId());
                Log.e("getDocuments", "meeting_document/list?lessonId=" + meetingConfig.getLessionId() + ",result:" + response);
                if (response == null) {
                    ...
                    try {
                        ...
                        Log.e("asyncGetDocumentDetail", "data:" + data);
                        if (data != null) {
                            int changeNumber = data.getInt("changeNumber");
                            meetingConfig.setChangeNumber(changeNumber);
                            JSONArray array = data.getJSONArray("documentList");
                            if (array != null) {
                                Gson gson = new Gson();
                                Log.e("array", "array:" + array);
                                List<MeetingDocument> documents = new ArrayList<>();
    ...
    public static void asyncGetDocumentsInDocAndRefreshFileList2(final MeetingConfig meetingConfig, final int itemId, final int pageNumber) {
        new ApiTask(new Runnable() {
            @Override
            public void run() {
                JSONObject response = ConnectService.getIncidentbyHttpGet(AppConfig.URL_PUBLIC + "Lesson/Item?lessonID=" + meetingConfig.getLessionId());
    ...
    public static EventRefreshDocs asyncGetDocuments2(final MeetingConfig meetingConfig) {
