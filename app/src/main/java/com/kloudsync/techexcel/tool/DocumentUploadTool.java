...
public class DocumentUploadTool {
	...
	public void setUploadDocImageRequestBody(int changeNumber, int liveImageId, int pageNumber) {
        mChangeNumber = changeNumber;
        mLiveImageId = liveImageId;
    ...
    public void uploadVideo(final Context context, String targetFolderKey1, int field1,
                            File file, int spaceID) {
        this.mContext = context;
        this.targetFolderKey = targetFolderKey1;
        this.field = field1;
        ...
        fileHash = Md5Tool.getMd5ByFile(mfile);
        isNeedConvert = false;
        this.fileName = mfile.getName();
        this.type = 0;

//        this.MD5Hash = Md5Tool.transformMD5(AppConfig.UserID + mfile.getName())+ System.currentTimeMillis();
        this.uploadLoadFullName = Md5Tool.getUUID() + mfile.getName();
        int index = this.uploadLoadFullName.lastIndexOf(".");
        if(index > 0){
            this.fileNamecon = this.uploadLoadFullName.substring(0,index);
        }else {
            this.fileNamecon = this.uploadLoadFullName;
        }

        this.MD5Hash = targetFolderKey + "/" +this.uploadLoadFullName;
        Log.e("check_target","MD5Hash:" + this.MD5Hash);
        LoginGet lg = new LoginGet();
        lg.setprepareUploadingGetListener(new LoginGet.prepareUploadingGetListener() {
            @Override
            public void getUD(Uploadao ud) {
...
//                                            }
//                                    );
                                    updateNewFileByType(convertingResult);
                                }
                            }
...
//        MD5Hash = Md5Tool.transformMD5(name2);
       /* PutObjectRequest put = new PutObjectRequest(ud.getBucketName(),
        ...
        Log.e("oss", " bucketname: "+ud.getBucketName()+" ,MD5Hash:  "+MD5Hash+"  "+",absolute_path:" + mfile.getAbsolutePath() + ",recordDirectory");
...
//                                updateGetListener.Update();
        ...
        if (convertingResult.getCurrentStatus() == 0) {  // prepare
            ...
            if (type != 1) {
                ...
                if (type == 10) {
                    ...
                    ServiceInterfaceTools.getinstance().uploadNewImage(ServiceInterfaceTools.UPLOADNEWIMAGE, mChangeNumber, convertingResult.getFileName(), convertingResult.getFileSize(), fileHash,
		                    mLiveImageId, field, mPageNumber, fileName, new ServiceInterfaceListener() {
                                @Override
                                public void getServiceReturnData(Object object) {
                                    Log.e("UploadNewFile", "object:" + object);
                                    if (uploadDetailLinstener != null) {
    ...
    private void updateNewFileByType(final ConvertingResult convertingResult){
        if (type != 1) {
