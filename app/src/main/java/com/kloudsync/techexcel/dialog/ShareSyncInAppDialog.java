...
public class ShareSyncInAppDialog implements View.OnClickListener, HeaderRecyclerAdapter.OnItemClickListener, IRongCallback.ISendMessageCallback {
    ...
    public void onSuccess(Message message) {
        new CenterToast.Builder(mContext).setSuccess(true).setMessage(mContext.getResources().getString(R.string.share_success)).create().show();
        dismiss();
    }

    @Override
    public void onError(Message message, RongIMClient.ErrorCode errorCode) {
        new CenterToast.Builder(mContext).setSuccess(false).setMessage(mContext.getResources().getString(R.string.share_fail)).create().show();
        dismiss();
    }
