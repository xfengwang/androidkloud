...
//                            uploadFile2(attachmentBean);
    ...
    public static void addVideoDocumentToSpace(final Activity activity, String filePath, final int spaceId, final DocumentUploadTool.DocUploadDetailLinstener uploadDetailLinstener) {
        final JSONObject jsonobject = null;
        String url = null;
        final File file = new File(filePath);
        final String fileHash = Md5Tool.getMd5ByFile(file);
                ...
                public void run() {
                    try {
                        ...
                        if (retcode.equals(AppConfig.RIGHT_RETCODE)) {
                            ...
                            DocumentUploadTool uploadTool = new DocumentUploadTool(activity);
                            uploadTool.setUploadDetailLinstener(uploadDetailLinstener);
                            uploadTool.uploadVideo(activity, targetFolderKey, field, file, spaceId);
                        } else if (retcode.equals(AppConfig.Upload_Exist + "")) { //不要重复上传
                            msg.what = AppConfig.FAILED;
                            msg.obj = errorMessage;
                            activity.runOnUiThread(new Runnable() {
